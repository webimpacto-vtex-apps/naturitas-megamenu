...
class Item extends PureComponent {
    ...
    render() {
        ...
        const itemLink =
        (!this.state.showItemDropdown && typeof item.rows !== 'undefined' && this.props.item.rows.length > 0)
        ...
        return (
            <li className={"nav-item " + (typeof item.rows !== 'undefined' && item.rows.length > 0 ? 'dropdown' : '') + (this.state.showItemDropdown ? ' show ' : '')}
            ...
            >
                ...
                {typeof item.rows !== 'undefined' && item.rows.length > 0 &&
