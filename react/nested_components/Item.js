...
class Item extends PureComponent {
  ...
  showItemDropdown = show => {
    ...
    if (this.dropdown && this.dropdown.current) {
      originalHeight = document.querySelector(
        `#${this.dropdown.current.id} .dropdown-container`
      ).offsetHeight
  ...
  render() {
    ...
    const itemLink =
      !this.state.showItemDropdown && this.props.item.rows.length > 0 ? (
        <a
          className={'nav-link' + (item.isGroupMobile ? ' menuTitle ' : '')}
          to={item.url}
          id="navbarDropdown"
    ...
    return (
      <li
        className={
          'nav-item ' +
          (item.rows.length > 0 ? 'dropdown' : '') +
          (this.state.showItemDropdown ? ' show ' : '')
        }
      ...
      >
        ...
        {item.rows.length > 0 && (
          <div
            id={'dropdown-menu' + this.props.keyNumber}
            className={
              'dropdown-menu ' +
              (this.state.showItemDropdown ? ' show ' : '') +
              (this.state.isSectionDropdownShow ? ' blockYScroll ' : '') +
              (item.isGroupMobile ? 'isGroup' : '')
            }
            ref={this.dropdown}
          ...
          >
            {!item.isGroupMobile && (
              <div
                className={'return col-lg-3 d-lg-none'}
                onClick={e => this.showItemDropdown(false)}
              >
            ...
            {!item.isGroupMobile && (
              <div className={'d-lg-none col-lg-3'}>
                ...
                {item.url && (
                  <Link
                    to={item.url}
                    className={'viewAllSection d-block'}
                    onClick={this.handleMenu}
                  >
                    {translates && translates.verTodo}{' '}
                    <span>
                      {item.title &&
                        item.title
                          .toLowerCase()
                          .charAt(0)
                          .toUpperCase() + item.title.toLowerCase().slice(1)}
                    </span>
                  </Link>
                )}
              </div>
