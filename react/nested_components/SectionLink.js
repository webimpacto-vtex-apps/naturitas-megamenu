  ...
  render() {
    ...
    return (
      <Link to={link.linkURL} className={link.linkImage ? 'hasImage' : ''}>
        <div
          onClick={e => this.props.handleMenu()}
          className={'sectionLink ' + (link.linkIcon ? ' hasIcon ' : '')}
        >
          {link.linkIcon && <img src={link.linkIcon} className="linkicon" />}

          {link.linkImage && <img src={link.linkImage} />}
